...
def main(boat_races_input):
    ...
    time = int("".join(lines[0].split("Time:")[1].strip().split()))
    record_distance = int("".join(lines[1].split("Distance:")[1].strip().split()))
    ...
    for hold in range(1, time + 1):
        ...
        if record_achieved > record_distance:
            ways.append(hold)
...
if __name__ == "__main__":
    boat_races = read_file("day_06_second_input.txt")
